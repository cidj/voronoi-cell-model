...
for i=1:ncycl,
    rann=rand(1);
    disp(['randp=',num2str(rann)]);
    p=floor(rann*dotnm)+1;
    %     p=floor(rand(1)*cellnm)+1;
     disp(['p=',num2str(p)]);
    rann=rand(1);
    disp(['randa=',num2str(rann)]);
    angle=rann*2*pi;
     disp(['angle=',num2str(angle)]);
    rann=rand(1);
    disp(['randpa=',num2str(rann)]);
    pace=rann*pace_am;
%     pace=0.4;
 disp(['pace=',num2str(pace)]);
    tempXY=XY(p,:);
    XY(p,:)=move(XY(p,1),XY(p,2),angle,pace);
        disp([' XY(p,1)=',num2str(XY(p,1))]);
        disp([' XY(p,2)=',num2str(XY(p,2))]);
...
%         XY(pp1:ppuni,:)=tempXY;
    ...
    sppaeap(i,3)=pace;
    sppaeap(i,4)=angle;
    sppaeap(i,5)=totenergy*beta;
    sppaeap(i,6)=deltaenergy*beta;
