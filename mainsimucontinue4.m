...
for i=1:ncycl,
    p1=movingcels(floor(rand(1)*length(movingcels))+1);
    ...
    condition1=checkallconnectionsu123( unitnm,vb_n,vb_next,dotnm);
    condition2=circleconnected(motcels,unitnm,va_n,vb_n,vb_next,dotnm,lengtha,lengthb);
    if(condition1&&condition2)
        cmel=fourcelledge(motcels,unitnm,va_n,vb_n,vb_next,dotnm,lengtha,lengthb);
    else
        cmel=nan;
    end
    setcondition=~isnan(cmel);
    if(setcondition)
%         disp(['cmel= ',num2str(cmel)]);
